...
import { MPanel } from "@rugo-vn/vue";
...
const loadData = async () => {
  ...
  for (let schema of infoStore.info)
    if (schema.__name === collectionName.value) fs.schema = schema;
};

watch(
  () => route.params.collectionName,
  async (name) => {
    if (route.name !== "FsCollection") return;
...
const handleRemove = async (docs) => {
  ...
  if (counter) noti.push("success", `Removed ${counter} item(s)!`);
...
const handleUpload = async (docs) => {
  ...
  if (counter) noti.push("success", `Uploaded ${counter} file(s)!`);
