...
import { MDashboardLayout } from "@rugo-vn/vue";
...
const updateCollectionRoute = async (name) => {
  ...
  for (let item of admin.navs) {
    if (item.name === formatedName) item.active = true;
...
watch(() => route.params.collectionName, updateCollectionRoute);
</script>
