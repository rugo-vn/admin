...
watch([() => props.schema, () => props.doc], reload);

reload();
</script>

<template>
  <h2 class="text-xl" v-if="props.schema.__name">
    {{ props.isCreate ? "Create a new" : "Edit an existed" }}
    {{ formatName(props.schema.__name).toLowerCase() }}
  </h2>

  <div v-for="field in fields" :key="field.key">
    <label class="block mb-2 mt-4 text-gray-600 capitalize">{{
      field.name
    }}</label>
