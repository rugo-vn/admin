...
export const formatValueWithSchema = (value, fieldSchema) => {
  ...
  switch (fieldSchema.type) {
    case "boolean":
      return `<ion-icon class="text-xl" name="${
        value ? "checkbox-outline" : "square-outline"
      }" />`;

    case "upload":
      if (value)
        return `<a class="hover:underline" href="${baseURL}${value}" target="_blank">${
          path.parse(value).base
        }</a>`;
      break;
